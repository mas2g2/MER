...
def main(filename,training=False,*args):
    ...
    textfilename = str(indatetime).replace(":","").replace(".","")+".txt"
   # logging.info("Saved the input signal into WAVE File")
    with sr.WavFile(filename) as source:
        ...
        textData = r.recognize_google(audio_data=audioData)       # Setting the language as English
        f = open(textfilename, "wt")
        f.write(textData)
        f.close()
...
if __name__ == '__main__':
    ...
    try:
        ...
        inputpath = input("Enter WAVE Filepath\n")
    if userinput == "1":
